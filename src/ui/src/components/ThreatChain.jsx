...
export default function ThreatChain({ alert }) {
  ...
  return (
    <div className="bg-slate-surface rounded-xl border border-white/5 shadow-[0_8px_32px_rgba(0,0,0,0.4)] relative overflow-hidden p-4 shrink-0">
      <div className="mb-4 flex flex-col xl:flex-row xl:items-center justify-between gap-2">
        <h3 className="text-2xl font-display font-medium">Event Timeline</h3>
        <p className="text-sm text-ghost font-mono bg-slate-bg inline-block px-3 py-1 rounded border border-white/5">
          Path: {mitreTags} <ChevronRight className="inline w-3 h-3 mx-1" /> {fatfTags}
        </p>
      </div>

      <div className="relative pb-2 overflow-x-auto custom-scrollbar">
        {/* Connecting line background */}
        <div className="absolute top-[44px] left-8 right-8 h-1 bg-gradient-to-r from-electric via-electric to-amber opacity-30 rounded-full" />
        ...
        <div className="flex items-start justify-between min-w-max gap-12 px-8 relative z-10">
          {threatChain.map((event, idx) => {
            ...
            return (
              ...
              >
                <div className="text-xs text-ghost font-mono mb-2">{timeStr}</div>
                
                <div className={`w-10 h-10 rounded-full bg-slate-bg flex items-center justify-center border-2 ${getBorderColor(event.type, alert.tls_risk_score)} shadow-lg shadow-black/50 group-hover:scale-110 transition-transform duration-300 ease-out relative`}>
